VERSION=$1
echo running tests for $VERSION of memcached
UUID=$(cat /proc/sys/kernel/random/uuid)
PAYLOAD='{"boxfile":{"name":"cache1","stability":"stable","version":'${VERSION}'},"logtap_host":"127.0.0.1","platform":"local","uid":"memcached1"}'
echo boxfile: "$PAYLOAD"

pass "unable to start the $VERSION container" docker run --privileged=true -d --name $UUID nanobox/memcached:$VERSION
